...
def test_d21_manifest_counts_mismatch_is_stop():
    man = json.loads((ROOT / "protocol" / "IPC_ELIGIBLE_PROBLEM_MANIFEST_v0.5.6.json").read_text())
    kept = man["eligible"]
    obs = counts_by_domain(kept)
    assert man["D21_status"] == STOP_IPC_OFFICIAL_BENCHMARK_MANIFEST_MISMATCH
    assert d21_verdict(obs) == STOP_IPC_OFFICIAL_BENCHMARK_MANIFEST_MISMATCH
    assert obs["Rover-GTOHP"] == 30
    assert obs["Satellite-GTOHP"] == 20
    assert obs["Transport"] == 40
    assert obs["Woodworking"] == 30
    assert obs["Woodworking"] != REQUIRED_COUNTS["Woodworking"]
    for row in kept:
