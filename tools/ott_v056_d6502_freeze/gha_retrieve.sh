...
OTT_REPORT_FREEZE_ID="$FREEZE_ID" \
OTT_REPORT_DIGEST="$DIGEST" \
OTT_REPORT_CONTENT_ROOT="$COMPUTED_ROOT" \
OTT_REPORT_SUPPLEMENT_REF="$SUPPLEMENT_REF" \
OTT_REPORT_RECEIPTS="$RECEIPTS" \
python3 - <<'PY'
import json
import os
import time
...
receipts = Path(os.environ["OTT_REPORT_RECEIPTS"])
...
freeze_id = os.environ["OTT_REPORT_FREEZE_ID"]
digest = os.environ["OTT_REPORT_DIGEST"]
root = os.environ["OTT_REPORT_CONTENT_ROOT"]
supplement_ref = os.environ["OTT_REPORT_SUPPLEMENT_REF"]
doc = {
